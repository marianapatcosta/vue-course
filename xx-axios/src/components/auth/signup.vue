      ...
      <form @submit.prevent="onSubmit">
        ...
        <div class="hobbies">
          ...
          <div class="hobby-list">
            <div class="input" v-for="(hobbyInput, index) in hobbyInputs" :class="{invalid: $v.hobbyInputs.$each[index].$error}" :key="hobbyInput.id">
              <label :for="hobbyInput.id">Hobby #{{ index }}</label>
              <input type="text" :id="hobbyInput.id" v-model="hobbyInput.value" @blur="$v.hobbyInputs.$each[index].$touch()"/>
...
export default {
  ...
  validations: {
    email: {
      //here we bind property required to the value required imported from  vuelidate
      required,
      email,
      unique: value => {
        // if '' returns true because other validators will evaluate this
        if (value === '') return true;
        /* return new Promise((resolve, reject) => {
          setTimeout(resolve(value !== 'test@test.com'), 1000);
        }); */
        return axios.get('/axios.json?orderBy="email"&equalTo="' + value + '"')
        .then(response => {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      //axiosInstance.post('/axios.json', formData)
